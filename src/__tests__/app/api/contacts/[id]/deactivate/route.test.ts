...
describe('POST /api/contacts/[id]/deactivate', () => {
  ...
  it('should successfully deactivate a contact', async () => {
    ...
    mockContactService.deactivateContact.mockResolvedValue(mockResult)
    ...
    const request = new NextRequest('http://localhost:3000/api/contacts/contact-1/deactivate', {
      method: 'POST',
      headers: {
    ...
    // Act
    const { POST } = await import('@/app/api/contacts/[id]/deactivate/route')
    ...
    expect(mockContactService.deactivateContact).toHaveBeenCalledWith(
      'contact-1',
      'user-1',
      requestBody
    )
  ...
  it('should return 400 when contact is already inactive', async () => {
    // Arrange
    const mockUser = createMockUser()
    const mockSession = { user: mockUser }
    vi.mocked(mockGetServerSession).mockResolvedValue(mockSession as any)
    ...
    expect(response.status).toBe(400)
    expect(data.success).toBe(false)
    expect(data.error).toBe('Contact is already inactive')
  })

  it('should return 400 when contact has pending activities', async () => {
    // Arrange
    const mockUser = createMockUser()
    const mockSession = { user: mockUser }
    vi.mocked(mockGetServerSession).mockResolvedValue(mockSession as any)
    ...
    expect(response.status).toBe(400)
    expect(data.success).toBe(false)
    expect(data.error).toBe('Cannot deactivate contact with pending activities')
  })
  ...
  it('should return 500 when service throws an error', async () => {
    ...
    mockContactService.deactivateContact.mockRejectedValue(new Error('Service error'))
